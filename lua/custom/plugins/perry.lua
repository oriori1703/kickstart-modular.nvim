...
function M.show_ts_node(opts)
  ...
  ---@param node TSNode
  ---@return TSNode[]
  local function get_parents(node)
    local res = {} ---@type table<TSNode>
  ...
  ---Create an s-expression for querying the list of nodes
  ---@param node TSNode
  ---@return string?
  local function get_name(node)
    for n, name in node:parent():iter_children() do
      if node:equal(n) then
        return name
      end
    end
    return nil
  ...
  ---@return string
  local function create_query(nodes, buffer)
    local res = string.format(') @test (#eq? @test "%s"', vim.treesitter.get_node_text(nodes[1], buffer))
    ...
    for _, node in ipairs(nodes) do
      ...
      res = string.format("%s (%s %s)", name_section, node:type(), res)
    end
    return res
  end
  ...
  for _, tree in ipairs(lang_tree:trees()) do
    ...
    if root and vim.treesitter.is_in_node_range(root, line, col) then
      ...
      local path = create_query(get_parents(node), bufnr)
...
return {
  ...
  config = function()
    vim.api.nvim_create_user_command("TSPerry", function()
      M.show_ts_node()
    end, {})
  end,
}
